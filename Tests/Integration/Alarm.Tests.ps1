...
<#
    The alarm type end to end, through the orchestrator.

    For an alarm file the month is decided per alarm, on 'alarm.raised'.
#>
...
BeforeAll {
    ...
    function New-AlarmRunHC {
        $testRoot = Join-Path $TestDrive "alarm_$(New-Guid)"
    ...
    function Get-AlarmRowFromExcelHC {
        param ($Run, [String]$MonthKey)

        @(
            Import-Excel -Path (Join-Path $Run.Excel "Alarms - $MonthKey.xlsx") `
                -WorksheetName 'alarms' -StartRow 2
        )
    }
}
...
Describe 'An alarm file with one record' {
    BeforeAll {
        $script:run = New-AlarmRunHC

        (New-AlarmXmlHC -Raised '2024-06-21T03:55:15+02:00').Save(
            (Join-Path $run.Xml 'Alarm.xml'))

        Invoke-AlarmRunHC -Run $run
    }
    ...
    It 'creates the Excel file of the month of its record' {
        Test-Path (Join-Path $run.Excel 'Alarms - 202406 June.xlsx') | Should-BeTrue
    }

    It 'writes the rows' {
        Get-AlarmRowFromExcelHC -Run $run -MonthKey '202406 June' |
        Should-BeCollection -Count 1
    }

    It 'archives the XML file' {
        Test-Path (Join-Path $run.Archive 'Alarm.xml') | Should-BeTrue
    }
}

Describe 'An alarm file with records in two months' {
    BeforeAll {
        $script:run = New-AlarmRunHC
        ...
        #>
        (New-AlarmXmlHC -Raised '2024-06-21T03:55:15+02:00', '2024-07-02T08:00:00+02:00').Save(
    ...
    It 'writes only the June record to the June file' {
        $rows = Get-AlarmRowFromExcelHC -Run $run -MonthKey '202406 June'

        $rows | Should-BeCollection -Count 1

        foreach ($row in $rows) {
            Get-RowMonthHC -Value $row.alarmRaised | Should-Be '202406 June'
        }
    }

    It 'writes only the July record to the July file' {
        $rows = Get-AlarmRowFromExcelHC -Run $run -MonthKey '202407 July'

        $rows | Should-BeCollection -Count 1

        foreach ($row in $rows) {
            Get-RowMonthHC -Value $row.alarmRaised | Should-Be '202407 July'
        }
    }
...
Describe 'An alarm file that is delivered twice' {
    BeforeAll {
        $script:run = New-AlarmRunHC
...
Describe 'An alarm file without a usable date' {
    BeforeAll {
        $script:run = New-AlarmRunHC

        (New-AlarmXmlHC -Raised '').Save((Join-Path $run.Xml 'NoDate.xml'))
