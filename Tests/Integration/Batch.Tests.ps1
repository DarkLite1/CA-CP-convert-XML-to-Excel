...
<#
    The batch type end to end, through the orchestrator.

    For a batch file the month is decided per delivery, on
    'deliveryHeader.load_start_date'. Fanout.Tests.ps1 covers the same split at
    a lower level, by calling the two operation scripts directly.
#>
...
BeforeAll {
    ...
    function New-BatchRunHC {
        $testRoot = Join-Path $TestDrive "batch_$(New-Guid)"
    ...
    function Get-BatchRowFromExcelHC {
        param ($Run, [String]$MonthKey)

        @(
            Import-Excel -Path (Join-Path $Run.Excel "Batches - $MonthKey.xlsx") `
                -WorksheetName 'deliveriesBatches' -StartRow 2
        )
    }
}
...
Describe 'A batch file with one record' {
    BeforeAll {
        $script:run = New-BatchRunHC

        (New-BatchXmlHC -LoadStartDate '2024-06-21T03:55:15+02:00').Save(
            (Join-Path $run.Xml 'Batch.xml'))

        Invoke-BatchRunHC -Run $run
    }
    ...
    It 'creates the Excel file of the month of its record' {
        Test-Path (Join-Path $run.Excel 'Batches - 202406 June.xlsx') | Should-BeTrue
    }

    It 'writes the rows' {
        Get-BatchRowFromExcelHC -Run $run -MonthKey '202406 June' |
        Should-BeCollection -Count 1
    }

    It 'archives the XML file' {
        Test-Path (Join-Path $run.Archive 'Batch.xml') | Should-BeTrue
    }
}

Describe 'A batch file with records in two months' {
    BeforeAll {
        $script:run = New-BatchRunHC
        ...
        #>
        (New-BatchXmlHC -LoadStartDate '2024-06-21T03:55:15+02:00', '2024-07-02T08:00:00+02:00').Save(
    ...
    It 'writes only the June record to the June file' {
        $rows = Get-BatchRowFromExcelHC -Run $run -MonthKey '202406 June'

        $rows | Should-BeCollection -Count 1

        foreach ($row in $rows) {
            Get-RowMonthHC -Value $row.loadStartDate | Should-Be '202406 June'
        }
    }

    It 'writes only the July record to the July file' {
        $rows = Get-BatchRowFromExcelHC -Run $run -MonthKey '202407 July'

        $rows | Should-BeCollection -Count 1

        foreach ($row in $rows) {
            Get-RowMonthHC -Value $row.loadStartDate | Should-Be '202407 July'
        }
    }
...
Describe 'A batch file that is delivered twice' {
    BeforeAll {
        $script:run = New-BatchRunHC
...
Describe 'A batch file without a usable date' {
    BeforeAll {
        $script:run = New-BatchRunHC
