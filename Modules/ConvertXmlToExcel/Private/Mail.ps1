...
<#
    .SYNOPSIS
        Build and send the summary e-mail.

    .DESCRIPTION
        Taken from the Permission-matrix project so this repository does not
        depend on the internal 'Toolbox.HTML' module. Mail is sent with the
        MailKit and MimeKit assemblies, whose paths come from the configuration
        file. These are .NET assemblies loaded with Add-Type, not PowerShell
        modules, so ImportExcel stays the only module dependency.
#>
